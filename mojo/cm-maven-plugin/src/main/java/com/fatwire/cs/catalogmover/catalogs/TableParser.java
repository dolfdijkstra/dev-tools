...
public class TableParser {
	...
	public TableData parseHTML(final String sBuffer) {
		...
		final String sVersion = extractBetweenTokens(sBuffer,
				m_sStartTableVersionTag, m_sEndTableVersionTag, ibrBufferPos);
		...
		if (m_nTableVersion >= TableData.BASETMLVERSION) {
			sWrapper = extractBetweenTokens(sBuffer, m_sWrapperStartTag,
					m_sWrapperEndTag, ibrBufferPos);
			...
			if (sWrapper.length() > 0) {
				m_sStartColumnTag2 = sWrapper + m_sStartID;
				m_sEndColumnTag2 = sWrapper + m_sEndID;
			} else {
				log
						.error("TableParser version/wrapper mismatch - required wrapper not found");
				m_sStartColumnTag2 = m_sStartColumnTag;
				m_sEndColumnTag2 = m_sEndColumnTag;
			}
		...
		nTableEnd = findTokenReverse(sBuffer, m_sEndTable);
		...
		sTableName = extractBetweenTokens(sTable, m_sStartTableNameTag,
				m_sEndTableNameTag, ibrBufferPos);
		if (sTableName == null) {
		...
		sTableType = extractBetweenTokens(sTable, m_sStartTableTypeTag,
				m_sEndTableTypeTag, ibrBufferPos);
		if (sTableType != null) {
			m_TableData.setTableType(sTableType);
		}

		sDBType = extractBetweenTokens(sTable, m_sStartDBTypeTag,
				m_sEndDBTypeTag, ibrBufferPos);
		if (sDBType != null) {
		...
		sRow = extractBetweenTokens(sTable, m_sStartRowTag, m_sEndRowTag,
				ibrBufferPos);
		...
		while (true) {
			...
			sRow = extractBetweenTokensSafe(sTable, m_sStartRowTag,
					m_sEndRowTag, ibrBufferPos);
	...
	protected int findTable(final String sBuffer) {
		...
		nFound = caselessFindStr(sBuffer, m_sBeginTable);
		...
		// ---
		// look for table end
		nFound = caselessFindStr(sBuffer, m_sEndTag, (nFound + m_sBeginTable
				.length()));
		if (nFound == -1) {
			return -1;
		}
		...
		return (nFound + m_sEndTag.length());
	}
	...
	protected void parseRow(final String sBuffer, final int nRow) {
		...
		sCell = extractBetweenTokens(sData, m_sStartColumnTag2,
				m_sEndColumnTag2, ibrEnd);
	...
	protected String parseSchema(final String sSchema,
		...
		if (nSep != -1) {
			...
			try {
				...
				} else if (sType.equalsIgnoreCase(binarydata)) {
	...
	protected boolean parseHeaderRow(final String sRow) {
		...
		ibrEnd = new IntStatus(0);
		sHeader = extractBetweenTokens(sRow, m_sStartHeaderColumnTag,
				m_sEndHeaderColumnTag, ibrEnd);
		...
		while (sHeader != null) {
			...
			sSchema = extractBetweenTokens(sHeader, m_sStartSchemaTag,
					m_sEndSchemaTag, ibrBogus);
		...
		// row for headers.
		if (nColumn == -1) {
			if (caselessFindStr(sRow, m_sStartBoldTag) != -1) {
				sHeader = extractBetweenTokens(sRow, m_sStartColumnTag,
						m_sEndColumnTag, ibrEnd);
				while (sHeader != null) {
					nColumn++;
					...
					// Extract the schema from the header record.
					ibrBogus = new IntStatus(0);
					sSchema = extractBetweenTokens(sHeader, m_sStartColumnTag,
							m_sEndHeaderColumnTag, ibrBogus);
	...
	protected String extractBetweenTokensSafe(final String sBuffer,
		...
		while (true) {
			...
			lastEndTag = caselessFindStrReverse(sBuffer, m_sEndColumnTag2, head);

			// look back for column start tag
			lastStartTag = caselessFindStrReverse(sBuffer, m_sStartColumnTag2,
					head);
			...
			if ((lastEndTag > -1 && lastStartTag > -1)
				...
				ibrStart.value = caselessFindStr(sBuffer, m_sEndColumnTag2,
						head);
				if (ibrStart.value == -1) {
					return sExtracted;
				}
		...
		while (true) {
			...
			// look back for column end tag
			lastEndTag = caselessFindStrReverse(sBuffer, m_sEndColumnTag2, tail);
