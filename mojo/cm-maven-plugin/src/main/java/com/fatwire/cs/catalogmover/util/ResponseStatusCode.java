...
public class ResponseStatusCode {
	...
	// the parts.
	private final StringBuffer result = new StringBuffer(Unknown);
	...
	void compose() {
		...
		if (StringUtils.goodString(sState)) {
			...
			state = bState ? successStr : failureStr;
		}
		...
		result.append(_params + p.toString() + sep);
		result.append(statusPostfix); // end comment
	}
	...
	public boolean setFromData(final String data) {
		try {
			...
			// will get tossed
			final int index = data.indexOf(statusPrefix);
	...
	public boolean setFromString(String s) {
		if (!StringUtils.goodString(s) || !s.startsWith(statusPrefix)) {
			return false;
		}

		try {
			s = s.substring(statusPrefix.length() + sep.length()); // past
			// header to
			// result,
			// past
			// first sep
			final List<String> parts = StringUtils.arguments(s, sep);
			...
			// first is result;
			String t = (String) parts.get(0);
			sState = t.substring(_result.length());
			if (sState.equals(successStr)) {
				bState = true;
				sState = null;
			} else if (sState.equals(failureStr)) {
				bState = false;
				sState = null;
			}

			// reason (user message)
			t = (String) parts.get(1);
			reason = t.substring(_reason.length());

			// error number
			t = (String) parts.get(2);
			t = t.substring(_err.length());
			final Integer ii = new Integer(t);
			err = ii.intValue();

			// command
			t = (String) parts.get(3);
			command = t.substring(_command.length());

			// params
			t = (String) parts.get(4);
			t = t.substring(_params.length());
