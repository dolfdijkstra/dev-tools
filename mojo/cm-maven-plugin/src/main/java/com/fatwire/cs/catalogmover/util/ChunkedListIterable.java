...
public class ChunkedListIterable<T> implements Iterable<List<T>> {
	 ...
	 *             if chunkSize is <= 0.
	 */
	public ChunkedListIterable(List<T> lst, int chunkSize) {
		if (chunkSize <= 0) {
	...
	private class ChunkedListIterator implements Iterator<List<T>>{
		 ...
		 */
		public List<T> next() {
			int nextEndPos = _start + _chunkSize; // Where is the next end
													// position (current
													// position + page size)
			_end = (nextEndPos > _lst.size()) ? _lst.size() : nextEndPos; // Did
																			...
																			// list's
																			// size?
			List<T> ret = _lst.subList(_start, _end); // Extract sublist
