...
<%! 
	...
	if (tt.intValue() < 1000) {
	    return Double.toString(tt.doubleValue())+" ms";
	} else if (tt.intValue() < 60000) {
  	    return Double.toString(tt.doubleValue()/1000)+" sec";
	} else {
            return Double.toString(tt.doubleValue()/(1000*60)) +" min";
	}
    }
%>
<cs:ftcs>
...
<center><h3>Resultset Cache Profiler</h3></center>
  <table class="altClass">
      ...
      if (ht != null) {
          ...
          %><td><%= df.format(stats.getCreatedDate())%></td><%
          ...
          %><td style="text-align:right"><%= getLengthString(ht.getTimeout())%></td><%
