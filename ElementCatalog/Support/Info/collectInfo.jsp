...
{ 
    printTableSectionTitle(out,"Servlet init parameters");
    java.util.Enumeration e0 = context.getInitParameterNames();
...
<%! private static Connection getConnection(String connectString, JspWriter out, String sUser, String sPasswd) throws Exception
{
    ...
    catch (Exception ex)
    {
        ...
        try
        {
            DataSource ds   =  (DataSource) ic.lookup("jdbc/" + connectString);
            printTableRow(out, "Trying again with ", "jdbc/" + connectString); 
            connection   =  ds.getConnection();
        }
        catch (Exception ex1)
        {
            printTableRow(out, "Failed to connect with jdbc/" + connectString, ex1.toString()); 
            try
            {
                DataSource ds = (DataSource) ic.lookup("java:" + connectString);
                printTableRow(out, "Trying again with ", "java:" + connectString); 
                connection = ds.getConnection();
            }
            catch (Exception ex2)
            {
                printTableRow(out, "Failed to connect with java:" + connectString, ex2.toString());
                try 
                {
                    printTableRow(out, "Trying again with" , " DriverManager "); 
                    connection = DriverManager.getConnection("jdbc:sybase:Tds:localhost:2638/ftcsdb", "ftuser", "ftuser");
                }
                catch (Exception ex3)
                {
                    printTableRow(out, "Failed to connect with DriverManager", ex3.toString());
                }
            }
        } 
    } 
...
<div class="left-column gray">
  <ul class="subnav divider">
      <a href="ContentServer?pagename=Support/Info/collectInfo#RequestH">Request Headers</a><br/>
      <a href="ContentServer?pagename=Support/Info/collectInfo#SystemP">System Properties</a><br/>
      <a href="ContentServer?pagename=Support/Info/collectInfo#Threads">Threads</a>
  </ul>
</div>
<%
...
<% 
 ...
 try
 {
  ...
  //printResponseHeaders(out, request);
...
<% } else { %>
   <div class="right-column">
        <div class="entry-header">
             <ics:callelement element="Support/Info/LoginFormInfo"/>
        </div>
   </div>
<% } %>
