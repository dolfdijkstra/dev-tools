        ...
        <div class="right-column">
            ...
            private void fileRollup(JspWriter out, String urlfile, String path, String tblname, String limit, String column, String key, String start) {
                try {
                    ...
                    out.print("&nbsp;<a href=\"ContentServer?pagename=Support/Audit/Default/BodyCheck&tblname="+tblname+"&limit="+limit+"&col="+column+"&key="+key+"&start="+start+"&deleteme=yes\">DeleteMe</a><br/>");
                    ...
                    for (int j=0; j<files.length; j++) {
                        ...
                        if (Utilities.isFolder(path+dirname+files[j])!=0) {
                            ...
                            if (xfile.equalsIgnoreCase(yfile)) {
                                ...
                                out.print("&nbsp;<a href=\"ContentServer?pagename=Support/Audit/Default/BodyCheck&tblname="+tblname+"&limit="+limit+"&col="+column+"&key="+key+"&filespec="+newpath+"&filepath="+oldpath+"&start="+start+"&useme=yes\">UseMe</a><br/>");
                            }
                        }
                    }
                } catch (Exception e) {
            ...
            <table class="altClass">
                <ics:listloop listname="bodies">
                    ...
                    <tr>
                        ...
                        <td><%= url %></td>
                        <td><font color="red"><b>NOT FOUND</b><br/><% fileRollup(out, url, defdir, tblName, ics.GetVar("limit"), ics.GetVar("col"), ics.GetVar("key"), ics.ResolveVariables("bodies.akey"));%></font></td>
                    </tr>
                    <% } %>
                </ics:listloop>	
            </table>
            ...
            <% } %>
            <% if (numrows==500) { %>
            <a href='ContentServer?pagename=<ics:getvar name="pagename"/>&tblname=<ics:getvar name="tblname"/>&limit=<ics:getvar name="limit"/>&col=<ics:getvar name="col"/>&key=<ics:getvar name="key"/>&start=<ics:resolvevariables name="bodies.akey" />'<b>==>><ics:getvar name="col"/></b> column - Next <ics:getvar name="limit"/> Rows</a><br/>
            <% } %>
            <%
            if (numrows > 0 && !foundBad){
            %>Scanned <%= numrows%> urlbodies and all were fine!<br/><br/><%
            }
            ...
            <table class="altClass">
                ...
                <ics:listloop listname="table" >
                    <tr>
                        ...
                        %><td><a href='ContentServer?pagename=<ics:getvar name="pagename"/>&tblname=<ics:getvar name="tblname"/>&limit=<ics:getvar name="limit"/>&col=<ics:listget listname="table" fieldname="COLNAME"/>&key=<ics:listget listname="table" fieldname="KEY"/>'><ics:listget listname="table" fieldname="COLNAME"/></a></td>
                        <% } else {
