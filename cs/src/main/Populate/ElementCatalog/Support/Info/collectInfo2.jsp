...
static class View {
	...
	void printWebXml(javax.servlet.ServletContext context) throws IOException{
	   String webXmlPath = context.getRealPath("/WEB-INF/web.xml");
		...
		printTableSectionTitle("web.xml");
	...
	void printAllPropertiesFiles(javax.servlet.http.HttpServletRequest request){
    ...
    void printAllIniFiles(String iniPath) throws IOException {
    
		File[] inifiles = new File(iniPath).listFiles(new IniFileFilter());
		java.util.Arrays.sort(inifiles);
		for (int i=0;i<inifiles.length; i++){
			printPropertyFile(inifiles[i]);
		}
	}

	void printPropertyFile(File iniFile) throws IOException {
		Properties props = new Properties();
		props.load(new FileInputStream(iniFile));
		Set keySet = new TreeSet(props.keySet());

		printTableSectionTitle("ini file: "+ iniFile.getName());
