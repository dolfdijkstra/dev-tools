...
String sql = "SELECT ttl, COUNT(id) AS pages FROM (SELECT DATEDIFF('mi',mtime,etime) ttl, id FROM SystemPageCache WHERE etime > CURRENT_TIMESTAMP) GROUP BY ttl ORDER BY ttl";
...
select max(pages) from (SELECT   COUNT ( * ) AS pages
              FROM   SystemItemCache where blobkey is null
          GROUP BY   id)


SELECT   pages , id
  FROM   (  SELECT   COUNT ( * ) AS pages, id
              FROM   SystemItemCache where blobkey is null
          GROUP BY   id
          ORDER BY   COUNT ( * ) DESC)
 WHERE   pages > 200
...
SELECT   pages , id
  FROM   (  SELECT   COUNT ( * ) AS pages, id
          ...
          GROUP BY   id
          ORDER BY   COUNT ( * ) DESC)
 WHERE   pages > (select (max(pages)/2) from (SELECT   COUNT ( * ) AS pages
              FROM   SystemItemCache where blobkey is null
          GROUP BY   id))



pm_notepublish.cs_text as notepublish
pm_cache_flush.cs_text as flushing
pm_publish_took.cs_text as publishing
pm_asset_took.cs_text as mirroring

'[TIME]Done with mirroring in ([[:digit:]]*) ms.'
'[TIME]Done flushing the cache on the target in ([[:digit:]]*) ms.'
'[TIME]Done with NotePublish in ([[:digit:]]*) ms.'
'[TIME]Publishing took ([[:digit:]]*) ms for ([[:digit:]]*) assets'
'[TIME]Done sending assets to the target'

SELECT
    cs_sessiondate as publish_date,
    ps.cs_sessiondate,
    pm_publish_took.cs_sessionid,
pm_notepublish.cs_text as notepublish,
pm_cache_flush.cs_text as flushing,
pm_publish_took.cs_text as publishing,
pm_asset_took.cs_text as mirroring
...
FROM
    PubMessage pm_notepublish,
    PubMessage pm_cache_flush,
    PubMessage pm_publish_took,
    PubMessage pm_asset_took,
    PubSession ps
WHERE
    ps.id = pm_notepublish.cs_sessionid
AND ps.id = pm_cache_flush.cs_sessionid
AND ps.id = pm_publish_took.cs_sessionid
AND ps.id = pm_asset_took.cs_sessionid
AND pm_notepublish.cs_text LIKE '[TIME]Done with NotePublish in %'
AND pm_cache_flush.cs_text LIKE '[TIME]Done flushing the cache on the target in %'
AND pm_publish_took.cs_text LIKE '[TIME]Publishing took %'
AND pm_asset_took.cs_text LIKE '[TIME]Done with mirroring in%'
AND cs_sessiondate > (sysdate-31)
ORDER BY cs_sessionid DESC
