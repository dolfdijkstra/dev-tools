...
<div class="entry">
     Useful to check build date and buid version number to make sure the latest release is installed.<br/>
...
<div class="entry-header">
     <h2><a href='<%= buildMenuUrl("GetVars") %>'>ContentServer Variables</a></h2>
</div>
<div class="entry">
...
<div class="entry">
     Checks SystemPageCache for missing files on disk. If records found, then there is the possibility the expire
</div>
<div class="entry-header">
     <h2><a href='<%= buildMenuUrl("AllTables") %>'>Full TableCount</a></h2>
</div>
<div class="entry">
    Counts rows for all tables in Systeminfo grouped by systable column. Useful for a general system audit.<br/>
        Other usages:
        <ul>
...
<div class="entry">
    Counts rows for all tables in Systeminfo grouped by Content Server's subsystem (namely assetframework, approval, publish, workflow, engage, etc.,). Useful for a general system audit.<br/>
        Other usages:
        <ul>
            <li>Useful for database caching entries in the configuration file (futuretense.ini)</li>
            <li>Url columns of tables other than elementcatalog can be verified (if file exists on disk or not)</li>
